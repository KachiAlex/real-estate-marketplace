...
// Only create HTTP server if running as standalone (not in Cloud Functions)
let server;
let io;
const PORT = process.env.PORT || 5000;

// Create server and Socket.IO only when needed (standalone or if required by dependency)
...
} // End of standalone server initialization

// Import models (always needed)
...
// Socket.IO connection handling (only for standalone server)
if (io) {
...
try {
  ...
  console.error('Failed to load admin chat routes:', error.message);
}
...
// But skip if running in Cloud Functions environment

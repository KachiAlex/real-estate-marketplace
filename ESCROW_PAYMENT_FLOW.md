...
The PropertyArk platform implements a comprehensive escrow payment system that ensures secure transactions between property buyers and vendors. The system uses Flutterwave as the payment gateway and includes automated dispute resolution and fund release mechanisms.
...
- If no action is taken within 14 days, transaction requires admin manual review and fund release
...
- `admin_review_required`: 14-day period passed, requires admin manual release
- `admin_released`: Admin manually released funds to vendor
- `refunded`: Admin refunded funds to buyer
...
### Admin
- Resolve disputes
- Release funds for expired escrow transactions (14+ days)
- Refund funds when appropriate
...
### Confirmation Timer
- 14-day countdown from payment date
- Real-time updates every minute
- Visual countdown display
- Admin review trigger after 14 days

### Admin Manual Release Process
- After 14 days without buyer action, transaction status changes to `admin_review_required`
- Admin reviews transaction and decides to:
  - **Release funds to vendor**: Status changes to `admin_released`
  - **Refund to buyer**: Status changes to `refunded`
- Admin can add notes explaining their decision
- Notification to all parties upon admin decision
- Status update to `admin_released` or `refunded`

...
const mockPropertyDocumentation = [
  {
    type: 'title-deed',
...
app.get('/api/admin/users', protect, authorize('admin'), async (req, res) => {
  try {
    ...
    filteredUsers.sort((a, b) => {
      ...
      // Handle date sorting
      if (sort === 'createdAt' || sort === 'lastLogin') {
        aVal = new Date(aVal);
        bVal = new Date(bVal);
      }
...
app.get('/api/agents', (req, res) => {
  try {
    ...
    // Filter agents (vendors with vendorCategory 'agent')
    let agents = mockUsers.filter(user => 
      user.role === 'vendor' && 
      user.vendorData && 
      user.vendorData.vendorCategory === 'agent'
    );
    
    // Filter by location if specified
    if (location && location !== 'all') {
      agents = agents.filter(agent => 
        agent.vendorData.agentLocation === location
      );
    }
...
// Blog admin routes
// Get all blogs (admin)
app.get('/api/admin/blogs', protect, authorize('admin'), async (req, res) => {
  try {
    const { page = 1, limit = 20, status, category, search, sort = 'createdAt', order = 'desc' } = req.query;
...
app.post('/api/admin/blogs', protect, authorize('admin'), sanitizeInput, validate, async (req, res) => {
  try {
    const blogData = {
      ...req.body,
      author: {
...
app.put('/api/admin/blogs/:id', protect, authorize('admin'), sanitizeInput, validate, async (req, res) => {
  try {
...
app.delete('/api/admin/blogs/:id', protect, authorize('admin'), async (req, res) => {
  try {
...
app.get('/api/admin/blogs/:id', protect, authorize('admin'), async (req, res) => {
  try {
    const blog = await Blog.findById(req.params.id)
      .populate('author.id', 'firstName lastName email avatar')
...
// Approve/reject blog comments (admin)
app.put('/api/admin/blogs/:blogId/comments/:commentId', protect, authorize('admin'), async (req, res) => {
  try {

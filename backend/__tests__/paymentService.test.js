...
describe('paymentService (unit)', () => {
  ...
  test('initializePayment creates a pending payment', async () => {
    ...
    const result = await paymentService.initializePayment({ user: { id: 'u1' }, amount: 200, paymentMethod: 'stripe', paymentType: 'escrow', relatedEntity: {}, description: 'desc', currency: 'NGN' });

    expect(db.Payment.create).toHaveBeenCalled();
    expect(result).toMatchObject({
      payment: { status: 'pending', amount: 200 },

...
const mockEscrowTransactions = require('../data/mockEscrowTransactions');
const mockDisputes = require('../data/mockDisputes');
const mockProperties = require('../data/mockProperties');

const getUserIdValue = (user) => user?.id || user;
...
const normalizeMockEscrowTransaction = (transaction = {}) => {
  const propertySnapshot = findMockPropertyById(transaction.propertyId);
  return {
    ...transaction,
    property: transaction.property || (propertySnapshot ? {
      id: propertySnapshot.id,
      title: propertySnapshot.title,
      price: propertySnapshot.price,
      location: propertySnapshot.location
    } : null),
    buyerName: transaction.buyerName || transaction.metadata?.buyerName || null,
    sellerName: transaction.sellerName || transaction.metadata?.sellerName || null,
    isMock: true,
    source: 'mock',
...
const filterMockEscrowTransactions = ({ user, status, type, search }) => {
  const userId = getUserIdValue(user);
  const isAdmin = user?.role === 'admin';
  const needle = search ? String(search).toLowerCase() : null;

  return (Array.isArray(mockEscrowTransactions) ? mockEscrowTransactions : [])
    .filter((transaction) => {
      ...
      if (type === 'seller') {
        if (!userId) return false;
        return transaction.sellerId === userId;
      }
      if (needle) {
        const propertySnapshot = findMockPropertyById(transaction.propertyId);
        const haystack = [
    ...
    .map(normalizeMockEscrowTransaction);
};

const normalizeMockDispute = (dispute = {}) => ({
  ...dispute,
  isMock: true,
  source: 'mock',
...
const filterMockDisputes = async ({ user, status }) => {
  const userId = getUserIdValue(user);
  const isAdmin = user?.role === 'admin';
  const mockList = Array.isArray(mockDisputes) ? mockDisputes : [];

  const involvedEscrowIds = new Set();
  if (!isAdmin && userId) {
    (Array.isArray(mockEscrowTransactions) ? mockEscrowTransactions : []).forEach((tx) => {
      if (tx.buyerId === userId || tx.sellerId === userId) {
        involvedEscrowIds.add(tx.id);
      }
    });
  }
  ...
  return mockList
    ...
    .filter((dispute) => {
      ...
      if (!userId) return false;
      if (!dispute.escrowId) return false;
      return involvedEscrowIds.has(dispute.escrowId) || dispute.initiatedBy === userId;
    })
    .map(normalizeMockDispute);
};

const isMissingDisputeTableError = (error) => {
  if (!error) return false;
  const sources = [error.message, error.parent?.message, error.original?.message, error.toString?.()]
    .filter(Boolean)
    .map((msg) => String(msg).toLowerCase());
  return sources.some((msg) => msg.includes('relation') && msg.includes('"dispute_resolutions"') && msg.includes('does not exist'));
};
...
class EscrowService {
  async listTransactions({ user, status, type, page = 1, limit = 20, search } = {}) {
    ...
    const toPlainTransaction = (sequelizeRow) => {
      const tx = sequelizeRow.toJSON();
      return {
        ...tx,
    ...
    const transactions = rows.map(toPlainTransaction).filter(matchesSearch);

    const mockTransactions = filterMockEscrowTransactions({ user, status, type, search });
    const combinedTransactions = [...transactions, ...mockTransactions];

    const realCount = search ? transactions.length : count;
    const totalItems = realCount + mockTransactions.length;
    const totalPages = Math.max(Math.ceil(totalItems / Number(limit)), 1);
    ...
    return {
      ...
      meta: {
        mockCount: mockTransactions.length,
        realCount: transactions.length
      }
    };
  }
  ...
  async updateStatus({ transactionId, status, user, notes }) {
    ...
    // Phase 3.2: Verify user has permission to update status
    const userId = user?.id || user;
    const userRole = user?.role || user;
    const isParticipant = userId === tx.buyerId || userId === tx.sellerId;
    const isAdmin = userRole === 'admin';
    
    if (!isParticipant && !isAdmin) {
      const error = new Error('You are not authorized to update this escrow transaction');
...
async listDisputes({ user, status, page = 1, limit = 20 } = {}) {
  ...
  if (!isAdmin) {
    ...
    const escrowIds = userEscrows.map((escrow) => escrow.id);
    where.escrowId = {
      [Op.in]: escrowIds.length ? escrowIds : ['__none__']
    };
  }
  ...
  let count = 0;
  try {
    const result = await db.DisputeResolution.findAndCountAll({
      where,
      attributes: {
        exclude: ['sellerResponse', 'sellerEvidence']
      },
      order: [['createdAt', 'DESC']],
      offset: (Math.max(Number(page), 1) - 1) * Number(limit),
      limit: Number(limit)
    });
    rows = result.rows;
    count = result.count;
  } catch (dbErr) {
    if (isMissingDisputeTableError(dbErr)) {
      console.warn('listDisputes: dispute_resolutions table missing, returning mock disputes only');
      rows = [];
      count = 0;
    } else {
      throw dbErr;
  ...
  const realDisputes = rows.map((row) => {
  ...
  const mockDisputesList = await filterMockDisputes({ user, status });
  const combinedDisputes = [...realDisputes, ...mockDisputesList];
  combinedDisputes.sort((a, b) => {
    const aTime = new Date(a.createdAt || a.updatedAt || 0).getTime();
    const bTime = new Date(b.createdAt || b.updatedAt || 0).getTime();
    return bTime - aTime;
  });

  const totalItems = count + mockDisputesList.length;
  const pagination = {
    currentPage: Number(page),
    itemsPerPage: Number(limit),
    totalItems,
    totalPages: Math.max(Math.ceil(totalItems / Number(limit)), 1)
  };

  return {
    rows: combinedDisputes,
    count: totalItems,
    pagination,
    meta: {
      realCount: count,
      mockCount: mockDisputesList.length
    }
  };
}
...
async submitSellerResponse({ transactionId, sellerResponse, sellerEvidence = [], user }) {
  ...
  if (!transactionId) {
    const error = new Error('Escrow transaction ID is required');
    error.statusCode = 400;
    throw error;
  }

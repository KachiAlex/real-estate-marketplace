describe('Dev server overlay debug', () => {
  it('captures webpack-dev-server overlay text if present and logs console errors', () => {
    ...
    // Try to find overlay, but don't fail if it doesn't exist
    cy.get('body').then(($body) => {
      const $iframes = $body.find('iframe#webpack-dev-server-client-overlay');
      
      if ($iframes.length === 0) {
        cy.log('NO_OVERLAY_FOUND - This is expected in production builds');
        return;
      }

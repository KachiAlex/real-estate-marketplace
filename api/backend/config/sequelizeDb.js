...
// Silence pg sslmode compatibility warnings in tests while preserving behavior.
if (
  ...
  DATABASE_URL += `${DATABASE_URL.includes('?') ? '&' : '?'}uselibpqcompat=true`;
}
...
const sequelizeOptions = {
  ...
  pool: isServerless ? {
    ...
    max: 1, // Single connection per function execution
    min: 0,
    acquire: 10000, // Faster acquire for serverless
    idle: 5000, // Shorter idle time
    evict: 5000 // Evict idle connections quickly
  } : {
    // Standard server pool settings
    max: 5,
    min: 0,
    acquire: 30000,
    idle: 10000
...
// Skip database initialization during tests to avoid hanging on external DB dependency
if (process.env.NODE_ENV !== 'test') {

 ...
 * - Removes all HTML/script tags (XSS protection)
 * - Validates length constraints (1-5000 characters)
 * - Trims whitespace
 * @param {string} content - Raw message content
 * @returns {Object} - { isValid: bool, content: string, error?: string }
 */
const sanitizeMessageContent = (content) => {
  ...
  // Sanitize HTML/script tags using xss library
...
router.post('/start', authenticateToken, async (req, res) => {
  try {
    ...
    // Sanitize and validate initialMessage
    const messageValidation = sanitizeMessageContent(initialMessage);
    ...
    try {
      ...
      if (!existingInquiry) {
        ...
        await db.PropertyInquiry.create({
          ...
          propertyImage,
          inquiryType: 'message',
          message: messageValidation.content,
          status: 'pending'
        });
    ...
    // Return both chatId formats for flexibility in frontend
    return res.json({
      success: true,
      chatId: chatId,  // propertyId-participant1Id-participant2Id format for frontend lookup
      conversationId: conversation.id,
      messageId: message.id,
      message: 'Message sent successfully',
      notification: 'Your message has been sent to the vendor',
      // Include these for explicit frontend routing
      buyerId: buyerId,
      vendorId: vendorId,
      propertyId: propertyId,
      participantIds: {
...
router.post('/:chatId/messages', authenticateToken, async (req, res) => {
  try {
    ...
    // Sanitize and validate message content
    const messageValidation = sanitizeMessageContent(content);
    if (!messageValidation.isValid) {
      return res.status(400).json({
        success: false,
        error: messageValidation.error
...
router.put('/:chatId/messages/:messageId', authenticateToken, async (req, res) => {
  try {
    ...
    // Store original content and update message with sanitized content

...
const INSPECTION_NOTIFICATION_TYPES = [
  'inspection_requested',
  'inspection_confirmed',
  'vendor_proposed_new_time',
  'buyer_rescheduled',
  'inspection_cancelled_by_buyer',
  'inspection_cancelled_by_vendor',
  'inspection_declined'
];
const UUID_V4ISH_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

const isValidUuid = (value) => typeof value === 'string' && UUID_V4ISH_REGEX.test(value.trim());
...
// @desc    Persist inspection notifications created client-side
// @route   POST /api/notifications/inspection
// @access  Private (Buyer/Vendor)
router.post('/inspection',
  protect,
  sanitizeInput,
  validate([
    body('notifications').isArray({ min: 1, max: 10 }).withMessage('notifications array (max 10) is required'),
    body('notifications.*.type').isIn(INSPECTION_NOTIFICATION_TYPES).withMessage('Invalid inspection notification type'),
    body('notifications.*.title').trim().isLength({ min: 1, max: 200 }).withMessage('Title is required'),
    body('notifications.*.message').trim().isLength({ min: 1, max: 500 }).withMessage('Message is required'),
    body('notifications.*.recipientId').optional().isString().trim().isLength({ min: 1 }).withMessage('recipientId must be a string id'),
  ...
  async (req, res) => {
    try {
      ...
      // Avoid failing the full batch because one client sends an invalid sender id.
      if (!isValidUuid(senderId)) {
        return res.status(400).json({
          success: false,
          message: 'Authenticated user ID must be a valid UUID for inspection notification sync',
          data: {
            saved: 0,
      ...
      for (const entry of notifications) {
        const recipientId = entry.recipientId || entry.recipient;
        if (!recipientId) {
          failed.push({
            type: entry.type,
        ...
        try {
          ...
          if (result.success) {
            ...
            failed.push({
              type: entry.type,
              recipientId,
              reason: result.message || 'save_failed'
            });
          }
        } catch (itemError) {
          failed.push({
            type: entry.type,
            recipientId,
            reason: itemError.message || 'save_failed'
          });
        }
      }
      ...
      console.error('Create inspection notification error:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to save inspection notifications'
      });
    }
  }
);

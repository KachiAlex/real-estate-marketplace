...
router.get('/admin', [
  ...
  try {
    console.log('📝 GET /api/blog/admin - Fetching admin blogs:', {
    ...
    console.log('✅ Retrieved', result.blogs?.length || 0, 'admin blogs');
    res.json({ success: true, data: result.blogs, total: result.total });
  } catch (error) {
    console.error('❌ Error listing admin blogs:', error.message, error.stack);
    res.status(500).json({ 
      success: false, 
      message: 'Server error',
      details: process.env.NODE_ENV === 'development' ? error.toString() : undefined
...
router.post('/', blogValidation, validate(), async (req, res) => {
  try {
    console.log('🚀 POST /api/blog - Creating blog with payload:', {
      title: req.body.title,
      titleLength: req.body.title?.length || 0,
      contentLength: req.body.content?.length || 0,
      excerptLength: req.body.excerpt?.length || 0,
      category: req.body.category,
      status: req.body.status,
      hasAuthorId: !!req.body.authorId,
      hasReqUser: !!req.user,
      userId: req.user?.id
    });

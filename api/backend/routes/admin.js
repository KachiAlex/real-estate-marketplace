...
// @access  Private (Admin only)
router.patch('/disputes/:id', protect, authorize('admin'), [
  body('status')
    .optional()
    .isIn(['open', 'in_review', 'resolved', 'escalated', 'closed'])
  ...
  body('resolution')
    .optional()
    .isIn(['buyer_favor', 'seller_favor', 'partial_refund', 'full_refund', 'rejected'])
  ...
  try {
    ...
    const escrowService = require('../services/escrowService.clean');
    const { status, resolutionNotes, resolution } = req.body;
    const disputeId = req.params.id;

    // Ensure we have admin notes
    const adminNotes = (resolutionNotes && resolutionNotes.trim()) || 'Dispute resolved by admin';

    // Handle rejection as a special case (sets status to closed without resolution)
    if (resolution === 'rejected' || status === 'closed') {
      const db = require('../config/sequelizeDb');
      const dispute = await db.DisputeResolution.findOne({
        where: { escrowId: disputeId }
      });
      ...
      // Call resolveDispute service for standard resolutions
      const updatedDispute = await escrowService.resolveDispute({
        transactionId: disputeId,
        resolution: resolution || 'seller_favor',
        adminNotes: adminNotes,
        user: req.user
      });

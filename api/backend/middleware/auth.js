...
// CRITICAL: Mock auth is DISABLED in production to prevent authentication bypass
// Mock auth is only allowed in development/testing environments
...
exports.protect = async (req, res, next) => {
  try {
    ...
    if (!token) {
      // CRITICAL: In production, NEVER allow mock auth fallback
    ...
    // CRITICAL: In production, mock tokens are NEVER accepted
    if (token && token.startsWith('mock-')) {
      if (process.env.NODE_ENV === 'production') {
        console.warn('[protect] ⛔ SECURITY ALERT: Mock token attempted in production environment');
        return res.status(401).json({
          success: false,
          message: 'Not authorized to access this route'
        });
...
// CRITICAL: Mock auth is disabled in production
exports.optionalAuth = async (req, res, next) => {
  try {
    ...
    if (!token) {
      // CRITICAL: Only allow mock auth in non-production environments
      if (process.env.NODE_ENV !== 'production') {
    ...
    // CRITICAL: In production, reject mock tokens immediately
    if (token.startsWith('mock-')) {
      if (process.env.NODE_ENV === 'production') {
        console.warn('[optionalAuth] ⛔ SECURITY ALERT: Mock token attempted in production');
        req.user = null; // Don't authenticate with mock token in production
        return next();
      }
      attachMockUserFromHeaders(req);
      return next();

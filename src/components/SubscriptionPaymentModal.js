  ...
  const initializePayment = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        // Handle new response structure with providerData
        const paymentUrl = result.data.providerData?.paymentUrl || result.data.paymentUrl;
        const reference = result.data.payment?.reference || result.data.reference;
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <button
                ...
                className="bg-blue-600 text-white h-11 min-h-[44px] px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium"
              >
                Retry
              </button>
            </div>

...
const Sidebar = () => {
  ...
  const { isMobileSidebarOpen, closeMobileSidebar, isDesktopSidebarEnabled } = useSidebar();
  ...
  const desktopVisibilityClass = isDesktopSidebarEnabled ? 'lg:translate-x-0' : 'lg:-translate-x-full';
  ...
  return (
    <>
      ...
      {/*
        WARNING: Do NOT add lg:translate-x-0 in the conditional below.
        desktopVisibilityClass already provides lg:translate-x-0 or lg:-translate-x-full.
        Adding another lg: class here will override it due to Tailwind's
        "last utility wins" rule, causing the sidebar to always show on desktop
        even on non-dashboard routes (home page, properties, etc.).
      */}
      {/* Sidebar */}
      <div className={`
        w-64 bg-white shadow-lg h-screen fixed left-0 top-0 z-[70] flex flex-col
      ...
      {/* User Account Footer - Compact: avatar, profile, logout */}
      {user && (
        <div className="flex-shrink-0 border-t border-gray-200 bg-white">
          <div className="px-4 py-3">
            <div className="flex items-center space-x-3 mb-3">
              {user.avatar || user.photoURL ? (
                <img src={user.avatar || user.photoURL} alt="avatar" className="h-9 w-9 rounded-full object-cover" />
              ) : (
                <div className="h-9 w-9 rounded-full bg-brand-blue text-white flex items-center justify-center text-sm font-medium">
                  {(user.firstName || user.displayName || user.email || 'U')[0].toUpperCase()}
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {user.firstName || user.displayName || (user.email && user.email.split('@')[0])}
                </p>
            ...
            <div className="space-y-1">
              <Link
                to="/profile"
                onClick={closeMobileSidebar}
                className="flex items-center space-x-3 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors"
              >
              ...
              <button
                onClick={logout}
                className="flex items-center space-x-3 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 w-full transition-colors text-left"
              >

...
const InvestmentModal = ({ 
  ...
  const validateForm = () => {
    ...
    if (!formData.amount || parseFloat(formData.amount) <= 0) {
      ...
      newErrors.amount = `Minimum investment is ${formatCurrency(investment.minimumInvestment)}`;
    } else if (parseFloat(formData.amount) > (investment.targetAmount - investment.currentAmount)) {
      newErrors.amount = `Maximum available is ${formatCurrency(investment.targetAmount - investment.currentAmount)}`;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/investments/${investment.investmentId}/invest`, {
        method: 'POST',
      ...
      toast.error(error.message || 'Failed to submit investment. Please try again.');
  ...
  const calculateOwnership = (amount) => {
    ...
    return ((numericAmount / investment.targetAmount) * 100).toFixed(2);
  };
          ...
          <div className="bg-blue-50 rounded-xl p-4 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-semibold text-blue-900">{formatCurrency(investment.targetAmount)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-semibold text-blue-900">{investment.expectedReturn}%</p>
              </div>
              <div>
                <span className="text-blue-700">Current Funding:</span>
                <p className="font-semibold text-blue-900">{investment.fundingPercentage}%</p>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  max={investment.targetAmount - investment.currentAmount}
                />
              </div>

...
const EscrowDashboard = ({ userRole = 'buyer' }) => {
  ...
  useEffect(() => {
    // Check for admin review required every minute
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'admin_review_required': return 'text-orange-600 bg-orange-100';
      case 'admin_released': return 'text-green-600 bg-green-100';
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'admin_review_required': return <FaExclamationTriangle className="h-4 w-4" />;
      case 'admin_released': return <FaCheckCircle className="h-4 w-4" />;
      case 'refunded': return <FaTimes className="h-4 w-4" />;
  ...
  const canTakeAction = (transaction) => {
    if (userRole === 'buyer') {
      ...
      return transaction.status === 'disputed' || transaction.status === 'admin_review_required';
    }
    return false;
  };

  const handleAdminRelease = async (transaction) => {
    const notes = prompt('Enter admin notes for fund release to vendor:');
    if (notes !== null) {
      await adminReleaseFunds(transaction.id, notes);
      setShowAdminActionModal(false);
    }
  };

  const handleAdminRefund = async (transaction) => {
    const notes = prompt('Enter admin notes for fund refund to buyer:');
    if (notes !== null) {
      await adminRefundFunds(transaction.id, notes);
      setShowAdminActionModal(false);
    }
  };
  ...
  const handleActionClick = (transaction) => {
    ...
    if (userRole === 'buyer') {
      ...
      if (transaction.status === 'admin_review_required') {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { 
            label: 'Completed', 
            value: filteredTransactions.filter(t => ['completed', 'buyer_confirmed', 'admin_released', 'refunded'].includes(t.status)).length,
            color: 'green'
          },
          { 
            label: 'Disputed', 
                          ...
                          <div className="mt-4 grid gap-3 md:grid-cols-2">
                            ...
                            <div className="p-4 rounded-2xl border border-rose-100 bg-rose-50">
                              ...
                              <ul className="mt-2 space-y-2 text-sm text-gray-700">
                                <li>
                                  <strong>Buyer:</strong> File a dispute from the confirmation modal with details/evidence before the 14-day window ends to pause fund release.
                                </li>
                                <li>
      ...
      {showAdminActionModal && selectedTransaction && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4">Admin Action Required</h3>
            <p className="text-gray-600 mb-6">
              The 14-day escrow period has expired for transaction {selectedTransaction.id}. 
              Please choose an action:
            </p>
            <div className="space-y-3">
              <button
                onClick={() => handleAdminRelease(selectedTransaction)}
                className="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
              >
                Release Funds to Vendor
              </button>
              <button
                onClick={() => handleAdminRefund(selectedTransaction)}

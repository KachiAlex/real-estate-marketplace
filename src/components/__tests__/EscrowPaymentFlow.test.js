...
jest.mock('../../services/socketService', () => ({
  ...
  joinEscrowRoom: jest.fn(),
  leaveEscrowRoom: jest.fn(),
  onEvent: jest.fn(),
  offEvent: jest.fn(),
}));
...
describe('EscrowPaymentFlow (Paystack)', () => {
  it('calls PaystackPop initializer when Paystack is selected and user continues to payment', async () => {
    ...
    // Click the current Paystack checkout button
    const launchBtn = screen.getByRole('button', { name: /Pay with Paystack/i });

...
const EscrowStatusTracker = ({ escrowTransaction }) => {
    ...
    const steps = [
      {
        ...
        icon: FaDollarSign,
        completed: ['funded', 'in_escrow', 'buyer_confirmed', 'admin_released', 'completed', 'disputed', 'cancelled'].includes(escrowTransaction.status)
      },
      {
        id: 'escrow',
        title: 'In Escrow',
        ...
        icon: FaShieldAlt,
        completed: ['in_escrow', 'buyer_confirmed', 'admin_released', 'completed', 'disputed', 'cancelled'].includes(escrowTransaction.status)
      },
      {
        id: 'confirmation',
      ...
      },
      {
        ...
        icon: FaCheck,
        completed: ['completed', 'admin_released'].includes(escrowTransaction.status)
      }
    ];
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'admin_released': return <FaCheck className="h-4 w-4" />;

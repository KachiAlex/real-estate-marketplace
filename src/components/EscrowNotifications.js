...
const EscrowNotifications = ({ userRole = 'buyer' }) => {
  ...
  const generateNotifications = () => {
    ...
    escrowTransactions.forEach(transaction => {
      ...
      // Admin review required (buyer/vendor)
      if (transaction.status === 'admin_review_required') {
        newNotifications.push({
          id: `admin-review-${transaction.id}`,
          type: 'urgent',
          title: 'Admin Review Required',
          message: `14-day escrow period expired. Admin must review and release funds for transaction ${transaction.id}`,
          transactionId: transaction.id,
          priority: 'urgent',
          timestamp: transaction.adminReviewRequiredAt || transaction.updatedAt,
          action: 'Review'
        });
      }

      // Admin released (buyer/vendor)
      if (transaction.status === 'admin_released') {
        newNotifications.push({
          id: `admin-released-${transaction.id}`,
          type: 'success',
          title: 'Funds Released by Admin',
          message: `Admin released funds for transaction ${transaction.id}`,
          transactionId: transaction.id,
          priority: 'medium',
      ...
      // Refunded (buyer/vendor)
      if (transaction.status === 'refunded') {
        newNotifications.push({
          id: `refund-${transaction.id}`,
          type: 'info',
          title: 'Funds Refunded',
          message: `Funds refunded to buyer for transaction ${transaction.id}`,
          transactionId: transaction.id,
          priority: 'medium',
          timestamp: transaction.completedAt || transaction.updatedAt,
          action: 'view'

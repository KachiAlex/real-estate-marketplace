...
export default function DashboardSwitch() {
  ...
  // 🔧 FIX: Track manual switch to prevent auto-sync override during role persistence
  ...
  useEffect(() => {
    ...
    if (lastManualSwitchRef.current.role === preferredRole && timeSinceManualSwitch < 2000) {
      console.log('⏸️ DashboardSwitch: Skipping auto-sync - manual switch in progress (', timeSinceManualSwitch, 'ms ago)');
      return;
    }
  ...
  const handleSwitch = async (targetRole, switchValueOverride = null) => {
    ...
    try {
      ...
      // 🔧 FIX: Record manual switch timestamp to prevent auto-sync override

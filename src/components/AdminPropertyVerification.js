...
const normalizeImages = (images = []) => {
  if (!Array.isArray(images)) {
    ...
    if (typeof images === 'object' && images && images.url) {
      return [images.url];
    }
  ...
  return images
    .map((img) => {
      ...
      if (typeof img === 'object' && img.url) {
        const url = typeof img.url === 'string' ? img.url.trim() : null;
        return url || null;
      }
      return null;
    })
...
const AdminPropertyVerification = () => {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'pending_payment': { color: 'bg-yellow-100 text-yellow-800', text: 'Pending Payment', icon: null },
      'payment_completed': { color: 'bg-blue-100 text-blue-800', text: 'Pending Review', icon: null },
      'approved': { color: 'bg-green-100 text-green-800', text: 'Verified ✓', icon: FaCheckCircle },
      'rejected': { color: 'bg-red-100 text-red-800', text: 'Rejected', icon: FaTimesCircle }
    };

    const config = statusConfig[status] || { color: 'bg-gray-100 text-gray-800', text: status, icon: null };
                        ...
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="text-xl font-bold text-gray-900">{request.propertyTitle}</h3>
                              {request.status === 'approved' && (
                                <FaCheckCircle className="text-green-600 text-lg" title="Verified Property" />
                              )}
                            </div>

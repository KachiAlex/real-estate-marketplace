...
import { useRefreshUserOnRouteChange } from './hooks/useRefreshUserOnRouteChange';
...
const PageWithSidebar = ({ children, id }) => {
  const { enableDesktopSidebar, disableDesktopSidebar, isDesktopSidebarEnabled } = useSidebar();

  useEffect(() => {
    enableDesktopSidebar();
    return () => disableDesktopSidebar();
  }, [enableDesktopSidebar, disableDesktopSidebar]);

  return (
    <div className="flex w-full">
      <main
        id={id}
        className={`flex-1 w-full transition-all duration-300 ml-0 ${isDesktopSidebarEnabled ? 'lg:ml-64' : ''}`}
        role={id ? 'main' : 'region'}
      >
        {children}
      </main>
...
function AppContent() {
  ...
  // Defer route change refresh to prevent blocking
  const refreshUserOnRouteChangeRef = useRef(false);
  useEffect(() => {
    if (!refreshUserOnRouteChangeRef.current) {
      refreshUserOnRouteChangeRef.current = true;
      // Delay initialization to allow app to render first
      setTimeout(() => {
  ...
  // Only refresh user after initialization
  ...
  // Sidebar is always rendered; visibility is controlled by PageWithSidebar via isDesktopSidebarEnabled
  ...
  // CSRF Protection (Phase 2.1)
  // Fetch CSRF token on app initialization (non-blocking for mobile compatibility)
  useEffect(() => {
    // Delay CSRF fetch to allow app to render first
                  ...
                  <MortgageProvider>
                    ...
                    {isAuthRoute && !isAuthModalRoute ? (
                      ...
                      <div className="flex flex-col min-h-screen w-full max-w-full overflow-x-hidden">
                        ...
                        <Sidebar />
                        <div className={`flex flex-grow w-full max-w-full overflow-x-hidden ${!shouldHideHeader && !isAuthModalRoute ? 'pt-20 sm:pt-24 md:pt-28' : ''}`}>
                          <ErrorBoundary>
                            <Suspense fallback={
                              <div className="flex items-center justify-center w-full h-screen">
                                <LoadingSpinner size="lg" />

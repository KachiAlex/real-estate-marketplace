...
const Properties = () => {
  ...
  const getPropertyThumbnail = (property) => {
    if (!property) {
      return 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=400&h=300&fit=crop';
    }
  ...
  const filteredProperties = useMemo(() => {
    ...
    if (appliedType) {
      ...
      const normalizedAppliedType = normalizeType(appliedType);
      filtered = filtered.filter(property => normalizeType(property.type) === normalizedAppliedType);
    }
  ...
  const filterOptions = useMemo(() => {
    // Normalize property types: capitalize first letter, remove duplicates
    const normalizeType = (type) => {
      if (!type) return '';
      const normalized = type.toLowerCase().trim();
      const typeMap = {
    ...
    const rawTypes = safeProperties.map(p => p.type).filter(Boolean);
    const normalizedTypes = rawTypes.map(normalizeType);
    const types = Array.from(new Set(normalizedTypes)).sort();

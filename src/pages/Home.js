...
const formatFilterCurrencyValue = (value) => {
  ...
  try {
    ...
    for (const threshold of thresholds) {
      if (Math.abs(amount) >= threshold.value) {
        const rounded = (amount / threshold.value).toFixed(1).replace(/\.0$/, '');
        return `₦${rounded}${threshold.suffix}`;
      }
    }
  ...
  const uniquePropertyTypes = useMemo(() => {
    ...
    allProperties.forEach(property => {
      if (property?.type) {
        ...
        const capitalizedType = property.type
          .split(' ')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
          .join(' ');
        typeSet.add(capitalizedType);
      }
    });
    
    // Return sorted unique types, with predefined types as fallback
    const uniqueTypes = Array.from(typeSet).sort();
    return uniqueTypes.length > 0 ? uniqueTypes : propertyTypes;
  }, [properties]);
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-center text-xs text-gray-300 mt-2">
              <span className="price-inline">{formatFilterCurrencyValue(0)}</span>
              <span className="text-orange-500 font-medium bg-gray-800 px-3 py-1 rounded price-inline">
                {formatFilterCurrencyValue(priceRange[0])} - {formatFilterCurrencyValue(priceRange[1])}
              </span>
              <span className="price-inline">{`${formatFilterCurrencyValue(Math.max(priceRange[1], 200000000))}+`}</span>
            </div>
    ...
    <>
                  ...
                  return (
                    <div key={cardId} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full">
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        >
                          <h3 className="font-semibold text-gray-900 mb-1 text-base sm:text-lg break-words overflow-wrap-anywhere">{property.title}</h3>
                          ...
                          <p className="text-gray-600 text-sm mb-3 break-words overflow-wrap-anywhere">
      ...
      {showMobileFilters && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-4">
          <div className="bg-gray-900 text-white w-full max-w-md max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-2xl border border-gray-700/60 shadow-xl p-4 sm:p-6">

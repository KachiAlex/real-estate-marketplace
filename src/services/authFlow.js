...
export const handlePostAuth = (result, navigate, setLoggedInUser, setShowRoleSelection) => {
  ...
  if (user && (user.role === 'mortgage_bank' || user?.roles?.includes('mortgage_bank'))) {
...
export const handleRoleSelection = async (
  ...
  try {
    ...
    // CRITICAL: Do NOT persist user data to localStorage - only update state
...
export const getDefaultRedirectPath = (user, redirectUrl) => {
  ...
  if (user.role === 'mortgage_bank' || user?.roles?.includes('mortgage_bank')) {

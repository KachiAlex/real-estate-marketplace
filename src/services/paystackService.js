...
export const initializePaystackPayment = async ({ email, amount, reference, metadata, onSuccess, onClose, publicKey }) => {
  try {
    ...
    toast.error(error.message || 'Unable to load Paystack at the moment. Please try again.');
    return;
  }
  ...
  if (!keyToUse) {
    ...
    toast.error('Paystack public key not configured (set REACT_APP_PAYSTACK_PUBLIC_KEY).');
    return;
  }

  if (!window.PaystackPop) {
    console.error('[Paystack] PaystackPop not available');
    toast.error('Paystack SDK not available. Please refresh and try again.');
    return;
  }

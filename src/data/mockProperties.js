const backendMockProperties = [
  {
    ...
    owner: { id: 'mock_vendor', firstName: 'Vendor', lastName: 'Example', email: 'vendor@example.com', phone: '+234-800-000-0000' },
    views: 45,
    isVerified: false,
...
const transformPropertyForFrontend = (prop) => ({
  ...
  agent: {
    name: prop.owner.firstName ? `${prop.owner.firstName} ${prop.owner.lastName}` : prop.owner.name,
  ...
  vendorId: prop.owner.id,
  vendorName: prop.owner.firstName ? `${prop.owner.firstName} ${prop.owner.lastName}` : prop.owner.name,
  vendorEmail: prop.owner.email,
  vendorPhone: prop.owner.phone,
  vendorCode: prop.owner.vendorCode || 'MOCK-0001'
});

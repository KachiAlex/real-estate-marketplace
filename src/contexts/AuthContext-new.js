...
const normalizeUser = (u) => {
  ...
  if (!activeRole) {
    // Priority: admin > vendor > mortgage_bank > buyer/user
    const rolePriority = ['admin', 'vendor', 'mortgage_bank', 'buyer', 'user'];
    activeRole = rolePriority.find((r) => normRoles.includes(r))
      || (u.role ? String(u.role).trim().toLowerCase() : null)
      || (u.userType ? String(u.userType).trim().toLowerCase() : null)
      || normRoles[0]
      || 'user';
  }
